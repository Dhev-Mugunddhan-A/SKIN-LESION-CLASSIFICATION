...
# Load the fitted preprocessor (pickled using scikit-learn 1.6.1)
with open("fitted_preprocessor_v2.pkl", "rb") as f:
    data_object = pickle.load(f)
st.write("Data preprocessor loaded successfully!")


st.title("Malignancy Prediction App")
st.write("Upload your metadata CSV string and an image, then click Predict.")

# --- CSV Metadata Input ---
st.header("Enter Metadata CSV")
...
if st.button("Predict"):
    if csv_input and uploaded_file is not None:
        try:
            # Run the model prediction on both image and metadata
        ...
        st.warning("Please provide both CSV metadata and an image.")
